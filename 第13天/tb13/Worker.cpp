...
void CWorker::PrintAll(SWorker** p)
{
	...
	while(*p)
	{
		cout << "\t\t\t\t\t\t" << (*p)->sName << "\t" << (*p)->nNumb << "\t" << (*p)->fSala << endl;
		p++;
	}
	system("pause");
}
...
void CWorker::SPrintAll()
{
	vector<SWorker>::iterator it = m_list.begin();
	if (!it[0].nNumb)
	{
		cout << "系统中无数据记录" << endl;
		system("pause");
	...
	{
		cout << "\t\t\t\t\t\t" << it[i].sName << "\t" << it[i].nNumb << "\t" << it[i].fSala << endl;
	}
	system("pause");
}
...
{
	bool(*Function[])(SWorker* &, SWorker* &) = {SortbyNumb,SortbyName,SortbySala};
	if (m_list.size() == 0)
		return NULL;
...
void CWorker::Save()
{
	...
	fwrite(&Super, sizeof(SWorker), 1, fp);
	for (int i = 0; i<(int)m_list.size(); i++)
...
void CWorker::Load()
{
	...
	long size=ftell(fp)/sizeof(SWorker)-1;
	rewind(fp);
	m_list.resize(size);
	/*m_list.clear();*/
	fread(&Super, sizeof(SWorker), 1, fp);
	SWorker* pData = m_list.data();
	for (int i = 0; i < size; i++)
		fread(pData++,sizeof(SWorker),1,fp);
	//int MAX = 100;
